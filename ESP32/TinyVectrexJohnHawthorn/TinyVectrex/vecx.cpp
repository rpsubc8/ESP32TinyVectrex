...
static unsigned int alg_rsh;  /* zero ref sample and hold */
...
//***********************************
int GetVECTREX_PDECAY()
{
 return VECTREX_PDECAY;
}
...
//***********************************
int GetSizeBytes_vectors_set()
{
 return sizeof(vectors_set);
}

//***********************************
int GetSizeBytes_vector_hash()
{
 return sizeof(vector_hash);
}
...
void snd_update()
{
 switch (via_orb & 0x18) 
 {
  case 0x00: break; //the sound chip is disabled 		 		
  case 0x08: break; //the sound chip is sending data 		 		
  case 0x10:
...
#ifdef use_lib_optimice_int_update
 #define int_update() if ((via_ifr & 0x7f) & (via_ier & 0x7f)){ via_ifr |= 0x80; } else { via_ifr &= 0x7f; }
#else
...
{
 #ifdef	use_lib_optimice_vecx_reset
...
#ifdef use_lib_optimice_call_via_sstep1
 #define via_sstep1() if ((via_pcr & 0x0e) == 0x0a){ via_ca2 = 1; } if ((via_pcr & 0xe0) == 0xa0){ via_cb2h = 1; }
#else
...
#endif
{
 ...
 goto start_emulation;
  ...
  start_via_sstep0:
	...
	if (via_srb < 8) {
		switch (via_acr & 0x1c) {
		case 0x00: break;//disabled			
		case 0x04: //shift in under control of t2			
			if (t2shift) {
				//shifting in 0s since cb2 is always an output
				via_sr <<= 1;
				via_srb++;
		...
		case 0x08:
			...
			via_srb++;
			break;
		case 0x0c: break;// shift in under cb1 control			
		case 0x10:
			// shift out under t2 control (free run)
			if (t2shift) {
				via_cb2s = (via_sr >> 7) & 1;
		...
		case 0x18:
			...
			via_srb++;
			break;
		case 0x1c: break; //shift out under cb1 control						
		}
